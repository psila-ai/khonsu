...
fn test_single_node_write_throughput() {
    ...
    println!("Single-node sequential write throughput: {:.2} ops/sec ({} operations in {:.2?})",
             throughput, operation_count, elapsed);
    
    // Ensure throughput is reasonable (adjust threshold as needed)
    assert!(throughput > 10.0, "Single-node write throughput too low: {:.2} ops/sec", throughput);
}
...
fn test_multi_node_write_throughput() {
    ...
    assert!(throughput > 10.0, "Multi-node write throughput too low: {:.2} ops/sec", throughput);
    assert!(success_rate > 90.0, "Multi-node write success rate too low: {:.2}%", success_rate);
}
...
fn test_read_throughput() {
    ...
    println!("Local read throughput: {:.2} ops/sec ({} operations in {:.2?})",
    ...
    println!("Remote read throughput: {:.2} ops/sec ({} operations in {:.2?})",
             throughput2, key_count, elapsed2);
    
    // Ensure throughput is reasonable
    assert!(throughput1 > 100.0, "Local read throughput too low: {:.2} ops/sec", throughput1);
    assert!(throughput2 > 50.0, "Remote read throughput too low: {:.2} ops/sec", throughput2);
}
...
fn test_mixed_workload_throughput() {
    ...
    let (elapsed, throughput) = measure_throughput(operation_count, || {
        ...
        let handle1 = thread::spawn(move || {
            for i in 0..ops_per_node {
                ...
                let is_read = (std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos() as usize % 100) < (read_ratio * 100.0) as usize;
                
                if is_read {
                    // Read operation
                    let key_idx = (std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos() as usize) % initial_key_count;
    ...
    println!("Mixed workload throughput: {:.2} ops/sec ({} operations in {:.2?})",
             throughput, operation_count, elapsed);
    println!("Read success rate: {:.2}% ({}/{})", read_success_rate, successful_reads, total_reads);
    println!("Write success rate: {:.2}% ({}/{})", write_success_rate, successful_writes, total_writes);
    
    // Ensure throughput and success rates are reasonable
    assert!(throughput > 10.0, "Mixed workload throughput too low: {:.2} ops/sec", throughput);
    assert!(read_success_rate > 90.0, "Read success rate too low: {:.2}%", read_success_rate);
    assert!(write_success_rate > 80.0, "Write success rate too low: {:.2}%", write_success_rate);
}
