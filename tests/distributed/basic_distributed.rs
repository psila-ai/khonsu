...
fn test_two_node_distributed_transaction() {
    ...
    assert!(
        data_exists_on_node(khonsu2, &key),
        "Data was not replicated to node 2 within timeout"
    );
...
fn test_distributed_transaction_abort() {
    ...
    assert!(
        result.is_none(),
        "Data should not exist after transaction abort"
    );
    ...
    assert!(
        result.is_none(),
        "Data should not be replicated after transaction abort"
    );
}
