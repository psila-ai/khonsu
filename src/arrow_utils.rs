...
pub fn merge_record_batches(
    ...
    let target_key_column_index = target
        .schema()
        ...
        .0;
    let target_key_array = target
        .column(target_key_column_index)
        .as_any()
    ...
    let mut new_arrays: Vec<Box<dyn arrow::array::ArrayBuilder>> = target_schema
        .fields()
        .iter()
    ...
    for i in 0..target.num_rows() {
        ...
        if let Some((_, update_batch)) = updates.iter().find(|(update_key, _)| update_key == key) {
            ...
            for col_index in 0..target_schema.fields().len() {
                ...
                append_value(
                    &mut new_arrays[col_index],
                    update_array,
                    0,
                    target_field.data_type(),
                )?; // Assuming update batch has only one row for this key
            }
            updated = true;
        }
    ...
    let final_arrays: Vec<ArrayRef> = new_arrays
        .into_iter()
