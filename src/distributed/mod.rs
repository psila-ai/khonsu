...
impl SerializableVersionedValue {
    ...
    pub fn from_versioned_value(
        value: &VersionedValue,
    ) -> Result<Self, crate::errors::KhonsuError> {
    ...
    pub fn to_versioned_value(&self) -> Result<VersionedValue, crate::errors::KhonsuError> {
        if self.is_tombstone {
            ...
            let reader =
                arrow::ipc::reader::FileReader::try_new(std::io::Cursor::new(&self.ipc_data), None)
                    .map_err(|e| crate::errors::KhonsuError::ArrowError(e.to_string()))?;
            ...
            let record_batch = reader
                ...
                .next()
                .ok_or_else(|| {
                    crate::errors::KhonsuError::ArrowError("No RecordBatch in IPC data".to_string())
                })?
                .map_err(|e| crate::errors::KhonsuError::ArrowError(e.to_string()))?;
...
impl fmt::Display for GlobalTransactionId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "Txn[Node:{},ID:{},TS:{}]",
            self.node_id, self.local_id, self.timestamp
        )
    }
}
...
pub mod prelude {
    pub use super::twopc::{
        Participant, ParticipantState, TwoPhaseCommitManager, TwoPhaseCommitPhase,
