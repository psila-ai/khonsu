...
impl Storage<ReplicatedCommit> for DistributedCommitStorage {
    fn append_entry(&mut self, entry: ReplicatedCommit) -> StorageResult<()> {
        ...
        self.db
            .put("last_log_index", last_idx_encoded)
        ...
        for op in ops {
            match op {
                StorageOp::AppendEntry(entry) => {
                    ...
                    // Update the last log index
                    let last_idx_encoded = bincode::serialize(&idx).map_err(|e| {
                        Box::new(std::io::Error::new(
                            std::io::ErrorKind::InvalidData,
                ...
                StorageOp::AppendEntries(entries) => {
                    ...
                    if current_idx > 0 {
                        ...
                        let last_idx_encoded = bincode::serialize(&last_idx).map_err(|e| {
                            Box::new(std::io::Error::new(
                                std::io::ErrorKind::InvalidData,
                                format!("Failed to serialize last log index: {}", e),
                            ))
                ...
                StorageOp::AppendOnPrefix(from_idx, entries) => {
                    ...
                    if current_idx > 0 {
                        ...
                        let current_last_idx = match self.db.get("last_log_index") {
                            Ok(Some(bytes)) => {
                                bincode::deserialize::<usize>(&bytes).map_err(|e| {
                                    Box::new(std::io::Error::new(
                                        std::io::ErrorKind::InvalidData,
                ...
                StorageOp::SetPromise(promise) => {
                    let encoded = bincode::serialize(&promise).map_err(|e| {
                        Box::new(std::io::Error::new(
                            std::io::ErrorKind::InvalidData,
                ...
                StorageOp::SetAcceptedRound(round) => {
                    let encoded = bincode::serialize(&round).map_err(|e| {
                        Box::new(std::io::Error::new(
                            std::io::ErrorKind::InvalidData,
                ...
                StorageOp::SetStopsign(stopsign) => {
                    if let Some(stopsign) = stopsign {
                        let encoded = bincode::serialize(&stopsign).map_err(|e| {
                            Box::new(std::io::Error::new(
                                std::io::ErrorKind::InvalidData,
    ...
    fn get_log_len(&self) -> StorageResult<usize> {
        let last_idx = match self.db.get("last_log_index") {
            Ok(Some(bytes)) => bincode::deserialize::<usize>(&bytes).map_err(|e| {
                Box::new(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                    format!("Failed to deserialize last log index: {}", e),
                ))
    ...
    fn get_decided_idx(&self) -> StorageResult<usize> {
        match self.db.get(KEY_DECIDED_IDX) {
            Ok(Some(bytes)) => Ok(bincode::deserialize::<usize>(&bytes).map_err(|e| {
                Box::new(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
    ...
    fn set_decided_idx(&mut self, idx: usize) -> StorageResult<()> {
        ...
        self.db.put(KEY_DECIDED_IDX, encoded).map_err(|e| {
            Box::new(std::io::Error::new(
                std::io::ErrorKind::Other,
    ...
    fn get_accepted_round(&self) -> StorageResult<Option<Ballot>> {
        match self.db.get(KEY_ACCEPTED_ROUND) {
            Ok(Some(bytes)) => Ok(Some(bincode::deserialize::<Ballot>(&bytes).map_err(
                |e| {
                    Box::new(std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
    ...
    fn set_accepted_round(&mut self, round: Ballot) -> StorageResult<()> {
        ...
        self.db.put(KEY_ACCEPTED_ROUND, encoded).map_err(|e| {
            Box::new(std::io::Error::new(
                std::io::ErrorKind::Other,
    ...
    fn set_promise(&mut self, promise: Ballot) -> StorageResult<()> {
        ...
        self.db.put(KEY_PROMISE, encoded).map_err(|e| {
            Box::new(std::io::Error::new(
                std::io::ErrorKind::Other,
    ...
    fn set_stopsign(&mut self, s: Option<StopSign>) -> StorageResult<()> {
        if let Some(stopsign) = s {
            ...
            self.db.put(KEY_STOPSIGN, encoded).map_err(|e| {
                Box::new(std::io::Error::new(
                    std::io::ErrorKind::Other,
            ...
            self.db.delete(KEY_STOPSIGN).map_err(|e| {
                Box::new(std::io::Error::new(
                    std::io::ErrorKind::Other,
    ...
    fn get_stopsign(&self) -> StorageResult<Option<StopSign>> {
        match self.db.get(KEY_STOPSIGN) {
            Ok(Some(bytes)) => Ok(Some(bincode::deserialize::<StopSign>(&bytes).map_err(
                |e| {
                    Box::new(std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
    ...
    fn set_compacted_idx(&mut self, idx: usize) -> StorageResult<()> {
        ...
        self.db.put(KEY_COMPACTED_IDX, encoded).map_err(|e| {
            Box::new(std::io::Error::new(
                std::io::ErrorKind::Other,
    ...
    fn set_snapshot(
        &mut self,
        snapshot: Option<<ReplicatedCommit as omnipaxos::storage::Entry>::Snapshot>,
    ) -> StorageResult<()> {
        if let Some(snapshot) = snapshot {
            let encoded = bincode::serialize(&snapshot).map_err(|e| {
                Box::new(std::io::Error::new(
            ...
            self.db.put(KEY_SNAPSHOT, encoded).map_err(|e| {
                Box::new(std::io::Error::new(
                    std::io::ErrorKind::Other,
            ...
            self.db.delete(KEY_SNAPSHOT).map_err(|e| {
                Box::new(std::io::Error::new(
                    std::io::ErrorKind::Other,
    ...
    fn get_snapshot(
        &self,
    ) -> StorageResult<Option<<ReplicatedCommit as omnipaxos::storage::Entry>::Snapshot>> {
        match self.db.get(KEY_SNAPSHOT) {
            Ok(Some(bytes)) => Ok(Some(
                bincode::deserialize::<<ReplicatedCommit as omnipaxos::storage::Entry>::Snapshot>(
                    &bytes,
                )
                .map_err(|e| {
                    Box::new(std::io::Error::new(
...
impl DistributedCommitStorage {
    ...
    pub fn recover_transaction_state(
        &self,
        txn_id: &crate::distributed::GlobalTransactionId,
    ) -> Result<Option<crate::distributed::TransactionState>> {
        // Scan the log for the transaction
        let compacted_idx = self
            .get_metadata::<usize>(KEY_COMPACTED_IDX)
            .map_err(|e| {
                KhonsuError::StorageError(format!("Failed to get compacted index: {}", e))
            })?
            .unwrap_or(0);

        let last_idx = self
            .get_metadata::<usize>("last_log_index")
